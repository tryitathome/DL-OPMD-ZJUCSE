import sys
sys.path.append(r"D:\MyWorkSpace\classify-LM\utils\__pycache__")
...
from models.metaformer_mae import *
...
def main():
    ...
    '''数据参数'''
    # 训练过程名字
    #train_process_name = 'JDR_class_20250223'
    train_process_name = 'JDR_FinalRunGen1_20250407'

    # 数据集路径和输出路径
    dataset_dir = 'JDR_class_20250223'
    output_dir  = 'output'
    ...
    # 是否断点重训
    resume      = False
    resume_path = "output\JDR20241207_convnextv2_tiny_HileakDataset\checkpoints\model_epoch_120.pth" #'output/caformer_b36_eva02_nopre/checkpoints/model_epoch_120.pth'

    # 是否加载预训练权重
    pretrained      = True
    pretrained_path = 'pretrained/convnextv2/convnextv2_tiny_22k_224_ema.pt'
    ...
    if resume:
        ...
        # 载入预训练参数
        if pretrained:
            pretrained_weights = torch.load(pretrained_path, map_location='cpu')['model']
            # 为不完全匹配的预训练权重设计
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print(f'Current max accuracy: {best_accuracy:.2f}%\n')
